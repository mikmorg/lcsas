...
> **STATUS: RESOLVED** — landed in `fc3ea83` (boot+recovery: record lcsas-init optical-only scan defect; spec sentinel fix [BOOT-06]); guarded by `tests/recovery_hardening/test_boot_path_quarantined.py`.
