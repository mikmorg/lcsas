...
> **STATUS: RESOLVED** — landed in `c2c3ee8` (recovery: regenerate 6 lcsas-ecc bins with the augment encoder [FMT-01]); guarded by `tests/integration/test_ecc_repair.py`.
...
**Implemented (increment 11 of N):** 2026-06-15. **RS03 format-conformance bug FIXED — cross-conformance gate now green, #321 closed.** The decoder parsed dvdisaster's geometry correctly but reported every data sector of a freshly `-mRS03`-augmented (undamaged) image as damaged, and even with that masked could not erasure-repair — so the only end-to-end proof against the REAL dvdisaster (`test_lcsas_ecc_repairs_dvdisaster_augmented_image`) was `xfail(strict)` pending #321. Three independent format divergences, each reverse-engineered from the pinned dvdisaster 0.79.10-pl6 source and verified byte-exactly against a real augmented image:

1. **CRC final-inversion.** dvdisaster's per-sector CRC (`src/crc32.c Crc32`) is the standard reflected CRC-32 (poly 0xEDB88320, init 0xffffffff) WITHOUT the conventional final `^0xffffffff` — it returns the raw running register (a 2048-byte zero sector hashes to 0x0E174561). `rs03_crc32` dropped its final inversion; `rs03.h`/`gf256` docs and `test_ecc.c`'s CRC vectors were updated to the un-inverted values (0x340BC6D9 for "123456789", 0xFFFFFFFF for empty, 0x0E174561 for a zero sector).
2. **CRC-layer chain-back ordering.** dvdisaster writes the CRC layer one position out of phase (`src/rs03-create.c`, the encoder thread + `flush_crc`): the CRC of the data sector at codeword position `pos`/layer `L` is stored in CRC sector **`(pos-1) mod sectorsPerLayer`**, slot `L` (the last CRC sector wraps to hold position 0's CRCs, dvdisaster's `firstCrc` cache). `read_stored_crc` (and the two self-encoders, `test_ecc.c` build_image + `ecc_make_fixture.c`) now use `(pos + spl - 1) % spl`. Empirically confirmed: every slot of CRC sectors 0,1,2,5,1408 matches the "next position" data sector's un-inverted CRC. Also: dvdisaster CRC-protects EVERY data-layer sector — including the ECC header sector and the padding region — so verify no longer skips them (both verified matching on a real image).
3. **Reed-Solomon parameters + position convention.** The decoder used first-root 0 / consecutive powers; dvdisaster uses the CCSDS choice **RS_FIRST_ROOT=112, RS_PRIM_ELEM=11, RS_PRIMTH_ROOT=116** (`src/dvdisaster.h`) and treats layer `L` as codeword polynomial position **254-L** (sym[0] = highest degree). `decode_codeword` was rewritten as a faithful port of dvdisaster's `src/rs03-fix.c` error+erasure decoder (Horner syndromes, erasure-seeded Berlekamp-Massey, Chien search keyed by PRIMTH_ROOT, Forney with the FCR-1 correction term, in-place XOR apply). Proof: the syndromes of every undamaged codeword vanish only under FCR=112/PRIM=11/254-L. Added `gf_index_of`/`gf_alpha_to` (with `alpha_to[GF_ALPHA0]=0`) to gf256 to expose dvdisaster's index/exp convention for the port.

**Result:** the committed x86_64 bin and the host build verify a freshly dvdisaster-augmented image clean (exit 0), detect 20 damaged sectors after corruption, and repair them BYTE-IDENTICALLY; over-capacity damage fails loud (exit 1, no partial write). The `xfail(strict)` marker was removed (the test now passes outright); `.github/workflows/ecc-weekly.yml` updated from "currently xfail(strict) pending #321" to a hard gate. **Validation:** `make -C recovery test` green (`test_ecc: OK`, `ALL OK` — CRC vectors updated, GF/layout/round-trip/uncorrectable still pass); cross-conformance `test_lcsas_ecc_repairs_dvdisaster_augmented_image` PASSES (PATH+LCSAS_ECC_REPAIR=1); always-on `test_ecc_selfrepair_no_dvdisaster.py` (2) + `test_dvdisaster.py` (32) pass; `make lint`/`make typecheck` clean; `make -C recovery fuzz-rs03-smoke` 0 crashes. All 6 cross bins regenerated (`make ecc-arches`); bin-parity byte-identical on the 3 Linux musl ecc bins, EXEMPT-rebuilt-OK on the macOS/Windows ecc bins (only the ecc bins were rebuilt/committed; the pre-existing non-ecc STALE bins are untouched). **Closes #321.** Remaining (non-blocking, as before): promote the opt-in cross-conformance to scheduled CI (GATE-plan dependency); restore.bat ECC e2e under wine (INFRA-01); optional phase-2 `lcsas-ecc augment` encoder.

**Implemented (increment 12 of N):** 2026-06-15. **restore.bat `--check-disc` ECC repair path validated end-to-end under wine** — the last named FMT-01 follow-up. New `tests/recovery_hardening/test_restore_bat_ecc_dispatch_wine.py` (2 tests, always-on when wine + a C compiler + the committed `lcsas-ecc.exe` are present; mirrors the Linux `test_restore_sh_ecc_dispatch.py` and the tiny-fixture `test_ecc_selfrepair_no_dvdisaster.py`). It builds a TINY real RS03 image with the in-repo fixture generator (no dvdisaster, sub-second), lays out a meta tree (`recovery/scripts/restore.bat` + the committed `recovery/bin/x86_64-pc-windows-gnu/lcsas-ecc.exe`), maps it as a wine drive, and runs the REAL `restore.bat --check-disc E:\disc.img` through wine's cmd: (1) clean image → "no damage found … intact"; (2) damaged image + `LCSAS_CHECK_DISC_AUTOFIX=1` → "DAMAGE DETECTED" then "repair succeeded", and the bytes on disk are asserted byte-identical to the original augmented image — i.e. the repair genuinely happened through wine → `lcsas-ecc.exe`, proving the now-RS03-conformant Windows binary (increment 11) drives the heir's scratched-disc path. Both tests pass (`2 passed in ~153 s`; wine prefix cold-start dominates). `make lint`/`make typecheck` clean. Same posture as the other wine tests: a strong local/CI-with-wine pre-filter; the authoritative Windows gate stays `windows-e2e.yml`. **All named FMT-01 follow-ups are now done; only the explicitly-optional phase-2 `lcsas-ecc augment` encoder (removes dvdisaster from the BURN side too) remains out of scope per the plan.**

**Implemented (increment 13 of N):** 2026-06-15. **Phase-2 `lcsas-ecc augment` ENCODER — dvdisaster removed from the burn side too; the codec is now bidirectionally conformant.** New `lcsas-ecc augment <image> [--out F]` (main.c `cmd_augment`) + `rs03_calc_layout()` (medium-ladder selection per dvdisaster's CalcRS03Layout: smallest fitting medium s.t. nroots ≥ 8; clip at 84 data layers / 170 roots) and `rs03_augment()` (rs03.c) which lays down the data region, writes the RS03 ECC header(s), computes the chain-back CRC layer, and fills the parity layers via the conformant `rs03_fix()` erasure path, padded to a full medium. **Bidirectional proof (both verified manually + as opt-in gates):** forward (increment 11) — lcsas-ecc reads REAL dvdisaster parity; reverse (this) — **REAL `dvdisaster -i out -t` recognises the embedded ECC ("Error correction properties … Ecc block test: pass", NOT "medium.ecc: not present"), and `dvdisaster -f` repairs 20 damaged sectors of an lcsas-ecc-augmented image BYTE-IDENTICAL using only OUR parity/CRC** ("CRC error in sector 50 … repaired == original: True"). Pipeline: `LcsasEccRunner.augment_iso` implemented (was decode-only NotImplementedError) — shells out to `lcsas-ecc augment`, atomic temp-then-rename, full-medium disk-space pre-flight; `select_ecc_runner(require_augment=True)` now returns the in-house runner when dvdisaster is absent, so **the burn pipeline can encode RS03 with NO dvdisaster installed** while the discs stay repairable by real dvdisaster. Tests: opt-in reverse differential `test_dvdisaster_repairs_lcsas_ecc_augmented_image` (LCSAS_ECC_REPAIR=1); always-on C unit `test_calc_layout` (spec §4.3: 1648 sectors → ndata 85 / nroots 170 / spl 1409 / total 359295 / firstEcc 119765); two stale unit tests updated to the new augment-capable contract (32 pass). **Validation:** `make -C recovery test` green (`test_ecc: OK`, incl. `test_calc_layout`); `make lint`/`make typecheck` clean; `make -C recovery fuzz-rs03-smoke` 0 crashes; reverse differential PASSES against real dvdisaster. All 6 cross bins regenerated (`make ecc-arches`). **FMT-01 is now FULLY COMPLETE — both decode and encode are in-house and bidirectionally dvdisaster-conformant; nothing remains out of scope.**

**Decision (2026-06-15): `lcsas-ecc augment` will NOT be performance-optimised — closed, not a follow-up.** The reverse differential took ~2h because augment fills parity via the erasure decoder rather than a dedicated RS encoder. That is deliberately acceptable: ENCODE only ever runs at WRITE time on the operator's machine, which has dvdisaster (the preferred, fast encoder — `select_ecc_runner` picks it first). The in-house augment is defence-in-depth / a correctness fallback, NOT on the durability-critical path. Only DECODE (verify/repair) is durability-critical — it runs at the heir's RESTORE time, decades out, on a bare machine with no dvdisaster — so that is the half that had to be in-house and is the half worth optimising. The augment-speed concern is therefore intentionally not pursued. (Rationale also recorded at `select_ecc_runner` in src/lcsas/ecc/dvdisaster.py.)
