...
- [ ] **K0.2** `src/lcsas/keyshare/` — pure-Python split() + combine() + per-share integrity/checksum. stdlib-only (no new runtime deps; matches the zero-dep ethos). deps: D0.1
- [ ] **K0.3** Known-answer vectors (official SLIP-0039 vectors, or self-generated GF(256) vectors checked into `tests/fixtures/keyshare/`) + property tests: any K-of-N reconstruct; any K-1 fail; single-bit share corruption is *detected*, not silently wrong. **100% line cov on `keyshare/`.** deps: K0.2
