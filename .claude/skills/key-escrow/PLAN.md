...
- [x] **C5.3** ASan/UBSan **clean** (lead re-ran). Test exercises 15 valid + 30 invalid vectors (error paths) + codec edges. **KEY-06 closed the follow-up:** coverage-c now filters in `src/lcsas-keyshare/.*` (slip39.c + main.c reported; main.c driven via real cards in Step 3d); `fuzz_slip39_mnemonic` added to `fuzz-smoke`/audit-gate; `test_keyshare` runs under `make sanitize` (in TEST_BINS); audit-gate CI path filter now includes `recovery/src/lcsas-keyshare/**`; EXEMPTIONS.md documents the dir. deps: C5.2
