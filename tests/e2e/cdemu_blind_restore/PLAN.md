...
In production source as `MediaType.TEST_TINY = (2_097_152, 0)` with
...
2 MB is the canonical test volume (raised from 1 MB in #142 to make room
for the holographic catalog + ISO 9660 overhead).  We deliberately shrink
the source data so a handful of discs can be filled from a few MB of
source data, keeping the fixture fast and the swap loop count high (the
