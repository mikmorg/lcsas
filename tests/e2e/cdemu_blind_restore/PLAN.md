...
│  ├── run LCSAS burn pipeline with TEST_CD media → ~10 ISOs                 │
...
Already in production source as `MediaType.TEST_CD = (104_857_600, 10)`
with `label_name == "CD"`. Disc labels are generated by
`generate_volume_label("LCSAS", "CD", N)` and look like:

    LCSAS_CD_2026_0001
    LCSAS_CD_2026_0002
    ...

100 MB is a "CD-shaped" test volume — not a real 700 MB CD. We deliberately
shrink so ~10 discs can be filled from ~900 MB of source data, keeping the
...
lcsas scan
lcsas stage --media TEST_CD --session blind-test
lcsas burn  --session blind-test
```

Output: `LCSAS_CD_2026_0001.iso` … `LCSAS_CD_2026_000N.iso` placed in the
...
```json
{
  ...
  "LCSAS_CD_2026_0001": "/var/lib/disc-vault/LCSAS_CD_2026_0001.iso",
  "...":                "..."
}
```
...
| 100 MB `TEST_CD` instead of real 700 MB CDs | 10 real CDs would force ~7 GB of source data and a 10 min burn. 100 MB × 10 is ~1 min and keeps the swap loop count high. |
...
1. ✅ `MediaType.TEST_CD` and `label_name` property — landed in
