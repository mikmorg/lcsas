...
# MUST point at an aarch64-ELF binary -- pointing at the host binary
# will fail under qemu-user with a confusing exec error.
RESTORE_BIN = Path(os.environ["LCSAS_RESTORE_BIN"]) if os.environ.get(
    "LCSAS_RESTORE_BIN"
) else REPO_ROOT / "recovery" / "bin" / "aarch64" / "lcsas-restore"
