"""Issue #119: tier-1 armv7 cross-built binary coverage via qemu-user.

The cross-built armv7 `lcsas-restore` binary is bundled on every
...
`recovery/bin/armv7/lcsas-restore`.  qemu-user-static + binfmt_misc
is preinstalled, so the ARM32 ELF is executed transparently — no
explicit `qemu-arm-static ./bin` wrapper needed.

If either the cross-built binary or `qemu-arm-static` is absent,
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        "armv7 tier-1 coverage requires both "
...
# ARM32-on-x86_64 user-mode emulation runs several times slower than
