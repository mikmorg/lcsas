...
| Item | Status | Notes |
|---|---|---|
| **Cross-platform meta-volume** | Phase 21.1–21.11 SHIPPED 2026-05-17; only 21.12 (macOS) pending | Multi-arch bundling for six targets (`x86_64-unknown-linux-musl`, `aarch64-unknown-linux-musl`, `armv7-unknown-linux-gnueabihf`, `aarch64-apple-darwin`, `x86_64-apple-darwin`, `x86_64-pc-windows-gnu`).  Tier 2 (rustic-static) + tier 3 (CPython) bundled for every target; tier 1 (`lcsas-restore`) bundled for every target except the two Darwin targets.  Phase 21.12 will close the macOS gap via osxcross — see [`CROSS_PLATFORM_META_RFC.md`](CROSS_PLATFORM_META_RFC.md) §6 Q6. |
