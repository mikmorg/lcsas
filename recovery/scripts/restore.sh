...
if [ -x "$RESTORE_BIN" ] && ! bin_preflight_ok "$RESTORE_BIN"; then
    ...
    if [ "$FALLBACK" = "1" ]; then
        ...
        # A wrong password / unreadable repo keys (exit 77) is TERMINAL:
        # tiers 2/2b/3 read the SAME keys with the SAME password, so they
        # would only fail identically -- and a later tier can leave a
        # partial tree behind before it rejects the password (issue #384).
        # Stop with a clear message instead of falling through.
        if [ $tier1_rc -eq 77 ]; then
            printf '[tier 1] wrong password or unreadable repo keys; the\n'  >&2
